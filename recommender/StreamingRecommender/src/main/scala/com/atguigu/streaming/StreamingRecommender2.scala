...
//    }
  ...
  def getSimilarMovies(redisClient: Jedis, mysqlClient: MySqlProxy, mid: Int, uid: Int, K: Int): Array[Int] = {
    ...
    // 获取指定电影最相似的K个电影
    // redisClient.smembers("set:" + mid): set(mid1:sim1, mid2:sim2, ...)
    // line: mid:sim
    // similarMoviesBeforeFilter: set(mid1, mid2, ...)
  ...
  def createUpdatedRatings(redisClient: Jedis, recentRatings: Array[(Int, Double)], candidateMovies: Array[Int]): Array[(Int, Double)] = {
    ...
    // allSimilars : [(cmovieId1, sim1_1),(cmovieId1, sim1_2),...,(cmovieId2, sim2_1),(cmovieId2, sim2_2),...]
  ...
  def main(args: Array[String]) {
    ...
    // dataDStream: DStream: RDD[(uid, mid, score, timestamp)]
    // msg: message[key, value[uid|mid|score|timestamp]]
    ...
    dataDStream.foreachRDD(rdd => {
      if (!rdd.isEmpty) {
        // RDD[(userId, movieId, score, timestamp)]
